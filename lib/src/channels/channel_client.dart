import 'package:youtubedart/src/reverse_engineering/pages/channel_about_369.dart';
...
class ChannelClient {
  ...
  /// [id] must be either a [ChannelId] or a string
  /// which is parsed to a [ChannelId]
  Future<Channel> get369(dynamic id) async {
    id = ChannelId.fromString(id);
    final channelPage = await ChannelPageAbout.get(_httpClient, id.value);

    return Channel(
        id,
        channelPage.channelDescription,
        channelPage.channelLogoUrl,
        channelPage.channelBannerUrl,
        channelPage.videosCount,
        channelPage.subscribersCount);
